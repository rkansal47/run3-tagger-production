...
########## Do JHUGen job! ##########
curl https://spin.pha.jhu.edu/Generator/JHUGenerator.v7.5.0.tar.gz --output JHUGenerator.tar.gz
tar xaf JHUGenerator.tar.gz

## modify the W mass to keep the nH/mW ratio
MH=${ARGS[0]}
echo MH $MH
MW=$(echo "scale=3; $MH / 125 * 80.399" | bc)
sed -i "s/M_W     = 80.399d0/M_W     = ${MW}d0/g" JHUGenerator.v7.5.0/JHUGenerator/mod_Parameters.F90
echo "%MSG-JHUGen modify the W mass to ${MW}"
...
./JHUGen ReadLHE=${JHUBASE}/cmsgrid_final.lhe DataFile=${JHUBASE}/cmsgrid_final_jhu.lhe ${JHUCMD}
