...
## 4. load use custom cmssw
## 5. download MG
## 6. use provided DIGI cfg
## 7. stop running dnntuples

## in additinal to cmsconnect_higgsbc version:
## 8. (for ggHWW) put new cards based on existing workflow
## 9. (for ggHWW) insert run_instMG_JHU.sh after untarring cmssw.tar.gz

sleep $(( ( RANDOM % 200 ) + 1 ))

wget --no-check-certificate https://coli.web.cern.ch/coli/tmp/.221023-010530_higgsbc_prod/inputs.tar.gz
tar xaf inputs.tar.gz
wget --no-check-certificate https://coli.web.cern.ch/coli/tmp/.221210-021730_gghww_prod/inputs_gghww.tar.gz
tar xaf inputs_gghww.tar.gz
rm -f inputs.tar.gz inputs_gghww.tar.gz
...
# insert run_instMG_JHU.sh to override run_instMG.sh
rm -f GeneratorInterface/LHEInterface/data/run_instMG.sh
rsync $WORKDIR/inputs/run_instMG_JHU.sh GeneratorInterface/LHEInterface/data/run_instMG.sh
...
cmsRun inputs/DIGIPremix_UL2017_template_cfg.py maxEvents=$NEVENT nThreads=$NTHREAD
