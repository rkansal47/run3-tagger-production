...
for mh in m_higgs:
    ...
    for mx in m_res:
        print('MX%.0f MH%.0f weight %.5f' % (mx, mh, mh_weight(mh)))
        generator.RandomizedParameters.append(
            cms.PSet(
                ConfigWeight = cms.double(mh_weight(mh)),
                GridpackPath =  cms.string('instMG://PairVectorLQ_LQToBTau/MG5_aMC_v2.6.5/%.0f:%.2f:%.0f' % (mh, mh / 100., mx)),
                ConfigDescription = cms.string('PairVectorLQ_LQToBTau_MLQ%.0f_WLQ%.2f_HTmin%.0f' % (mh, mh / 100., mx)),
