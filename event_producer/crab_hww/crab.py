...
def createConfig(args, dataset, private_mc=False):
    ...
    config.JobType.pluginName = 'Analysis' if not private_mc else 'PrivateMC'
...
def main():
    ...
    submit_failed = []
    request_names = {}
    logger.info("*** Please do voms-proxy-init first ***")
    with open(args.inputfile) as inputfile:
        for l in inputfile:
            ...
            cfg, cfgpath = createConfig(args, dataset, args.private_mc)
