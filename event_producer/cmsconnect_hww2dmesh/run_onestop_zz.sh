...
## 4. load use custom cmssw [rename the script..!]
## 5. download JHUGen (-> not MG)
## 6. use provided DIGI cfg
## 7. stop running dnntuples

sleep $(( ( RANDOM % 200 ) + 1 ))

wget --no-check-certificate https://coli.web.cern.ch/coli/tmp/.230401-030439_hww2dmesh_zz_prod/inputs.tar.gz
tar xaf inputs.tar.gz
rm -f inputs.tar.gz

wget --no-check-certificate https://spin.pha.jhu.edu/Generator/JHUGenerator.v7.5.0.tar.gz -O JHUGenerator.tar.gz
...
cp -f GeneratorInterface/LHEInterface/data/run_generic_tarball_cvmfs_JHUVariableZMassFixRatio.sh GeneratorInterface/LHEInterface/data/run_generic_tarball_cvmfs_JHUVariableZMass.sh
